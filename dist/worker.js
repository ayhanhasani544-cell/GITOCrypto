...
var extendString = /* @__PURE__ */ __name(function() {
  const _add = /* @__PURE__ */ __name((name, body2) => Object.defineProperty(String.prototype, name, _noEnum(body2)), "_add");
...
var extendUint8Array = /* @__PURE__ */ __name(function() {
  const _add = /* @__PURE__ */ __name((name, body2) => Object.defineProperty(Uint8Array.prototype, name, _noEnum(body2)), "_add");
  _add("toBase64", function(urlsafe) {
...
__name(validUserId, "validUserId");
function cents(value) {
  const n = typeof value === "number" ? value : Number(String(value));
  if (!Number.isFinite(n) || n < 0 || n > 1e12) throw new Error("invalid amount");
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/api/user/create") {
        ...
        if (!validUserId(b.userId)) return json({ error: "invalid userId" }, 400);
      ...
      if (request.method === "GET" && match) {
        ...
        const x = r.rows[0];
        return json({ userId: x.user_id, balance: `${usdt(x.balance_cents)} USDT`, balanceUsdt: usdt(x.balance_cents), walletAddress: x.wallet_address, network: x.network });
      }
      if (request.method === "POST" && url.pathname === "/api/earn") {
        if (!adminAuth(request, env)) return json({ error: "admin authentication required" }, 403);
        const b = await body(request);
        if (!validUserId(b.userId) || !b.providerEventId || !b.source) return json({ error: "userId, providerEventId and source required" }, 400);
        const amount = cents(b.amount);
        const id = crypto.randomUUID();
        const tx = await client.transaction("write");
        try {
          await tx.execute({ sql: `INSERT INTO earnings(id,user_id,amount_cents,source,provider_event_id) VALUES(?,?,?,?,?)`, args: [id, b.userId, amount, b.source, b.providerEventId] });
          await tx.execute({ sql: `UPDATE users SET balance_cents=balance_cents+?,updated_at=CURRENT_TIMESTAMP WHERE user_id=?`, args: [amount, b.userId] });
          await tx.commit();
        } catch (e) {
          await tx.rollback();
          return json({ error: "duplicate or invalid earning" }, 409);
        }
        return json({ earningId: id, userId: b.userId, amount: `${usdt(amount)} USDT` }, 201);
      }
      if (request.method === "POST" && url.pathname === "/api/withdraw") {
        const b = await body(request);
        if (!validUserId(b.userId) || !b.walletAddress || !["TRC20", "ERC20", "BEP20"].includes(b.network)) return json({ error: "invalid withdrawal data" }, 400);
        const amount = cents(b.amount);
        const r = await client.execute({ sql: `SELECT balance_cents FROM users WHERE user_id=?`, args: [b.userId] });
        if (!r.rows.length) return json({ error: "user not found" }, 404);
        if (amount < 1e3) return json({ error: "minimum withdrawal is 10.00 USDT" }, 400);
        if (Number(r.rows[0].balance_cents) < amount) return json({ error: "insufficient balance" }, 400);
        const id = crypto.randomUUID();
        const tx = await client.transaction("write");
        try {
          await tx.execute({ sql: `UPDATE users SET balance_cents=balance_cents-?,updated_at=CURRENT_TIMESTAMP WHERE user_id=? AND balance_cents>=?`, args: [amount, b.userId, amount] });
          await tx.execute({ sql: `INSERT INTO withdrawals(id,user_id,amount_cents,wallet_address,network,status) VALUES(?,?,?,?,?,'pending')`, args: [id, b.userId, amount, b.walletAddress, b.network] });
          await tx.commit();
        } catch (e) {
          await tx.rollback();
          return json({ error: "withdrawal failed" }, 409);
        }
        return json({ withdrawalId: id, status: "pending", amount: `${usdt(amount)} USDT` }, 201);
      }
      const wh = url.pathname.match(/^\/api\/withdrawals\/([^/]+)$/);
      if (request.method === "GET" && wh) {
        const r = await client.execute({ sql: `SELECT id,amount_cents,wallet_address,network,status,transaction_id,created_at FROM withdrawals WHERE user_id=? ORDER BY created_at DESC`, args: [wh[1]] });
        return json({ withdrawals: r.rows.map((x) => ({ ...x, amount: `${usdt(x.amount_cents)} USDT` })) });
      }
      const th = url.pathname.match(/^\/api\/transaction\/([^/]+)$/);
      if (request.method === "GET" && th) {
        const r = await client.execute({ sql: `SELECT * FROM transactions WHERE id=?`, args: [th[1]] });
        return r.rows.length ? json(r.rows[0]) : json({ error: "transaction not found" }, 404);
      }
